...
class historial extends padreM
{
	...
	/*	$query="SELECT * FROM
		...
		coalesce(fecha_limite, 0) as fecha_limite,
		coalesce(adeudos, 0) as adeudos,
		coalesce(ticket_pago, '') as ticket_pago,
		coalesce(monto, 0) as monto,
		coalesce(consumos_mes, 0) as consumos_mes,
		coalesce(saldo_favor, 0) as saldo_favor,
		coalesce(cuota_admin, 0) as cuota_admin,
		coalesce(cargos_add, 0) as cargos_add,
		coalesce(lectura_fin, 0) as lectura_fin,
		coalesce(ruta_pdf, 0) as ruta_pdf,
		coalesce(pagado, 0) as pagado,
		coalesce(fecha_pago, 0) as fecha_pago,
		coalesce(total_a_pagar, 0) as total_a_pagar,
		...
		coalesce(municipio, 0) as municipio,
		coalesce(colonia, 0) as colonia,
		coalesce(codigo_p, 0) as codigo_p,
	 id_cuenta
	FROM
		   departamentos as dep
	...
	LEFT JOIN edificios as edi
		 ON dep.id_edificio = edi.id_edificio
	where
	dep.id_edificio=$id_departamento
	order by dep.id_departamento asc;";
		$datos=$this->con->consultaRetorno($query);
		return $datos;
	}
	...
	public function total($id_edificio, $periodo){
		$query=" SELECT coalesce(sum(consumos_litros), 0) as total, periodo, lec.id_departamento, edi.num_edificio, consumo_m3
		FROM lectura as lec, edificios as edi, departamentos as dep
		where
		edi.id_edificio=dep.id_edificio
		and
		lec.id_departamento=dep.id_departamento
		and
		edi.id_edificio $id_edificio
		and periodo like '$periodo';";
		//sin igual por qu lo trae desde el controller
		$datos=$this->con->consultaRetorno($query);
		return $datos;
	...
	public function get_historial_id($id_lectura){
		$query="SELECT * FROM lectura as lect, departamentos as edi
		        where
		        lect.id_departamento=edi.id_departamento
	            and
